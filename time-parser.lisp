;;;-*-Mode: LISP; Package: TIME-PARSER -*-
;;;

;;; $Header: /home/gene/library/website/docsrc/pdl/RCS/time.lisp,v 1.3 2004/09/27 00:59:09 gene Exp $
;;;
;;; Copyright (c) 2004 Gene Michael Stover.  All rights reserved.
;;; Improvements Copyright (c) 2018 Shannon Spires. All rights reserved.
;;;
...
ISO 8601 format.  It's compact because it's all numbers (as required
...
ISO 8601 format.  It's verbose because it separates the fields
...
(defun rread-from-string (&rest args)
  "Safe version of read-from-string"
  (let ((*read-eval* nil))
...
convert to a number & return the number."
  (let ((x (next-fn strm #'(lambda (ch)
                             (or (digit-char-p ch)
                                 (char-equal ch #\.))))))
    (and x (car (multiple-value-list (rread-from-string x))))))
...
the string is 14 characters & they are all digits."
  ...
  (and (eql 14 (length str))
       ...
       (list (cons 'year (rread-from-string (subseq str 0 4)))
             (cons 'month (rread-from-string (subseq str 4 6)))
             (cons 'day (rread-from-string (subseq str 6 8)))
             (cons 'hour (rread-from-string (subseq str 8 10)))
             (cons 'minute (rread-from-string (subseq str 10 12)))
             (cons 'second (rread-from-string (subseq str 12))))))
...
as a string."
  ...
  (do ((lst    ()  (cons (read-char strm) lst))
       (count   0                   (1+ count)))
      ((or (>= count width) ; Was >. That bug prevented %Y%m%d from working.
           (end-of-stream? strm)
...
(defun parse-time-zone-minus-hour (strm hour)
  ...
  (let* ((min1 (when (xdigit? (peek-char nil strm nil))
    ...
    (cond ((and (xdigit? min1) (xdigit? min2))
           ...
           (cons :zone
                 (+ hour
                    (/ (rread-from-string (format nil "~C~C" min1 min2))
                       60))))
  ...
  (let* ((hour1 (when (xdigit? (peek-char nil strm nil))
    ...
    (cond ((and (xdigit? hour1) (xdigit? hour2))
          ...
          ((and (xdigit? hour1) (eql (peek-char nil strm nil) #\:))
           ...
           ;; go for the minutes.
           (parse-time-zone-minus-hour strm (- (rread-from-string
                                                (format nil "~C" hour1)))))
          ((xdigit? hour1) 
...
(defun parse-time-zone (strm)
  "I hate time zones.  A bitch to parse.  And a
...
(defparameter *term-parsers*
  (let ((ht (make-hash-table :test #'equal)))
    (setf (gethash "%%" ht) #'parse-percent-percent
          ...
          ;; %x means don't care. Discard everything to end of stream and return a literal empty string.
          (gethash "%x" ht) (lambda (strm) (loop while (read-char strm nil nil)) (cons :literal ""))
...
(defparameter *default-recognizers*
      (list (make-fmt-recognizer "%Y-%m-%dT%H:%M:%S")
            ...
            ; 'recognize-yyyymmdd ; no longer needed since we fixed %Y%m%d
            'recognize-verbose-iso
            (make-fmt-recognizer "%B%d,%Y")    ; silly American date
            (make-fmt-recognizer "%B%d%Y")     ; like above but no comma necessary
            (make-fmt-recognizer "%a%B%d,%Y")  ; Lisp style date "Wednesday December 22, 2004"
            (make-fmt-recognizer "%a%B%d%Y")   ; like above but without comma.
            (make-fmt-recognizer "%a,%B%d,%Y") ; Maximum commafication "Thursday, June 23, 2005"
            (make-fmt-recognizer "%d-%b-%Y")   ; like "20-Jan-2018"

            ; For UUCP messages
;Tue, 21 Jul 2009 09:23:37 -0700 (PDT)
            (make-fmt-recognizer "%a,%d%B%Y")
            (make-fmt-recognizer "%a,%d%B%Y%H:%M:%S")
            (make-fmt-recognizer "%a,%d%B%Y%H:%M:%S%Z")
            (make-fmt-recognizer "%a,%d%B%Y%H:%M:%S%Z%x")

            ; Others
            (make-fmt-recognizer "%a,%B%d%Y")  ; Last variant just for completeness.
            (make-fmt-recognizer "%m/%d/%Y")   ; stupid American date
            (make-fmt-recognizer "%d %m %Y") ; Like CIA world factbook indep_date
            (make-fmt-recognizer "%a %B %d %H:%M:%S %Y") ; like in a PDF info record
            ))

; Caches
(defparameter *previous-best-recognizer* nil)
(defparameter *other-recognizers* (copy-list *default-recognizers*))

(defun find-recognizer (str tokens)
  "Finds a recognizer that recognizes str/tokens args.
  Returns what that recognizer produces.
  Caches that recognizer for quick access on subsequent calls."
  (let ((result nil)
        (recognizer nil))
    (when *previous-best-recognizer*
      (setf result (funcall *previous-best-recognizer* str tokens)))
    (unless result
      (setf recognizer (find-if #'(lambda (fn)
                                    (setf result (funcall fn str tokens)))
                                *other-recognizers*)))
    (when result ; we found one that worked
      (when recognizer ; did we find it on *other-recognizers*?
        ; if so, move that one from *other-recognizers* to *previous-best-recognizer*
        ;    and push the old one from *previous-best-recognizers* (if any) onto *other-recognizers*
        (setf *other-recognizers* (remove recognizer *other-recognizers*))
        (when *previous-best-recognizer* (push *previous-best-recognizer* *other-recognizers*))
        (setf *previous-best-recognizer* recognizer))
      ; if not, then *previous-best-recognizer* worked again, so just leave it alone
      result)))
...
(defun parse-time (str)
  ...
  (let* ((tokens (tokenize str))
         (result (find-recognizer str tokens)))
    (when result ; if result is nil, no valid recognizer was found
      ...
      (typecase result
        (integer result)
        (broken-time (broken-to-ut result))
        (list (broken-to-ut (create-broken result)))
        (t nil)))))
